...
// Định nghĩa giao diện cho yêu cầu tìm kiếm danh mục
...
// Định nghĩa giao diện cho phản hồi tìm kiếm danh mục
interface SearchCategoryResponse {
    pageData: CategoryResponse[];
    pageInfo: null;
    searchInfo: null;
}

// Hàm tìm kiếm danh mục theo tên
const searchCategoryByName = async (name: string): Promise<CategoryResponse> => {
    const response: AxiosResponse<BaseResponse<SearchCategoryResponse>> =
        await axiosInstance.post(`/category/search`, {
            name,
            pageNum: 1,
            pageSize: 1,
    ...
    if (
        ...
        response.data.data.pageData.length === 0
    ) {
        throw new Error("Category not found");
    }
    return response.data.data.pageData[0];
};
...
const DecantPage: React.FC = () => {
    ...
    // Lấy danh mục "Nước hoa chiết"
    useEffect(() => {
        const fetchCategory = async () => {
            try {
                setLoadingCategory(true);
                const categoryData = await searchCategoryByName("Nước hoa chiết");
